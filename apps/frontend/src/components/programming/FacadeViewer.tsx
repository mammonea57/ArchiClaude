...
type FacadeType = "rue" | "arriere" | "lateral";

interface FacadeViewerProps {
  /** Raw SVG string for a facade from the API */
  svgContent: string;
  /** Which facade is displayed */
...
const FACADE_LABELS: Record<FacadeType, string> = {
  ...
  lateral: "Façade latérale",
};

export function FacadeViewer({ svgContent, facadeType = "rue" }: FacadeViewerProps) {
  ...
  return (
    <div
      ...
      className={[
        ...
        isFullscreen ? "fixed inset-0 z-50 rounded-none" : "h-[360px]",
      ].join(" ")}
    >
      {/* Toolbar */}

...
SECTIONS = [
    ("✅ 2026-07-02 U MONO-FAÇADE FINAL v2 — confort, 0 dégagement, circ 18,9%, MONTRABLE + FINANÇABLE 12,9%", "concept/nogent_80_heros_3_scenarios/renders_partis/U_MONOFACADE_R1.png",
     "Valide par le VERIFICATEUR AUTOMATIQUE universel : 0 bloquant, 0 mineur. 2 cages d'angle + couloir 1,20m (mini PMR). Logements MONO-FACADE cote COUR, CONFORT : sejour/cuisine = VRAI BLOC ouvert (fenetre+cuisine) qui distribue lui-meme les chambres -> ZERO degagement/couloir interne (regle user gravee au hook + verif). Chambres 3,1x4m / 2,8x4m cote cour (fenetre), service (entree/SdB/WC/cellier) au fond. Coins BORGNES du U = celliers/caves (rangement, hors circulation). 10 logts/etage, mix 20% T2 / 60% T3 / 20% T4, circulation 18,9% (<20%!), 0 borgne/tunnel/enfilade/vide/bloat. BILAN : 60 logts sur R+5, marge 12,89% TTC. Reste : integration pipeline BuildingModel (rendu 3D + confo PLU)."),
    ("🏗️ 2026-07-02 U TRAVERSANT (abandonné : exigeait trop de cages) — archive", "concept/nogent_80_heros_3_scenarios/renders_partis/U_TRAVERSANT_R1.png",
     "Remplissage optimise : 11 logts/etage (vs 9), circulation tombee a 7% (vs 25% avec les paliers gaspilles), 3 cages partagees. Chambres COUR (Ch PAR>enf), sejour RUE, DEGAGEMENT central (0 enfilade), 0 vide, 0 borgne, 0 tunnel. Tailles resserrees pres du spec [68,81,82,85,85,94]. Mix/etage T4:6 / T3:4 / T2:1 -> 55 logts sur R+5 (skew T4 car les ailes traversantes a 9m de frontage = T4 3-chambres ; T5 exigerait 12m). RESTE : (1) mix trop T4-lourd a rebalancer ; (2) angle NE en vrai bi-facade rue nord+est ; (3) INTEGRATION pipeline BuildingModel (Cellule/Room) pour bilan/conformite/3D -- encore proto geometrie standalone."),
    ("🏗️ 2026-07-01 PROTO layout TRAVERSANT (bandeau seul, dégagement central, chambres cour ≥3,4m, 0 enfilade)", "concept/nogent_80_heros_3_scenarios/renders_partis/PROTO_traversant_bandeau.png",
     "Preuve de concept du nouveau layout interne demandé : chambres cote COUR (calme/jardin, 13-17m2, larges 3,4m = fini les tunnels 1,5m), sejour cote RUE, DEGAGEMENT central filant qui distribue CHAQUE piece -> on va du salon a chaque chambre / SdB / cuisine SANS traverser une autre chambre (intimite). Cages esc+asc ponctuelles (1 sert 2 apts, entrees face a la cage) au lieu du couloir filant. Bandeau nord = T4/T5 (2 facades). Ailes = T2/T3 single-facade (layout dedie a faire). PAS ENCORE integre au plan U complet — validation du parti avant le gros chantier solver."),
